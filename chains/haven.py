...
class MockHaven(HttpClient):
    ...
    def create_wallets(self):
        for key in self.seeds:
            result = self.call_RPC("wallet", "restore_deterministic_wallet", {
                "filename" : key,
                "password" : "password",
                "seed" : self.seeds[key]
            })
    ...
    def get_balance(self, address):
        ...
        """
        # open wallet
        self.call_RPC("wallet", "open_wallet", {
            "filename" : self.wallets[address],
            "password" : "password"
        })
        ...
        # get balance
        result = self.call_RPC("wallet", "get_balance", {
            "account_index" : 0,
            "asset_type" : "XHV"
        })
    ...
    def transfer(self, txn):
        ...
        # create transaction
        result = self.call_RPC("wallet", "transfer", {
            "destinations" : [
                {
        ...
        txn.gas = [Coin("XHV.XHV", result["fee"])]
